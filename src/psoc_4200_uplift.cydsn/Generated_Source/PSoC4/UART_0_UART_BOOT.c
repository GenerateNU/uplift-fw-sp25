    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        do
        {
            ...
            if (0u != UART_0_SpiUartGetRxBufferSize())
            {
                ...
                do
                {
                    byteCount = UART_0_SpiUartGetRxBufferSize();
                    CyDelayUs(UART_0_UART_BYTE_TO_BYTE);
                }
                while (byteCount != UART_0_SpiUartGetRxBufferSize());

                byteCount = UART_0_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) UART_0_SpiUartReadRxData();
        }
    }
...
cystatus UART_0_UartCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        UART_0_SpiUartPutArray(pData, (uint32) size);
