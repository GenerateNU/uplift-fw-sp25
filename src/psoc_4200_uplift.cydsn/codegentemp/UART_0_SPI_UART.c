...
***************************************/
    ...
    */
    ...
    volatile uint8 UART_0_rxBufferInternal[UART_0_INTERNAL_RX_BUFFER_SIZE];
    ...
    *
    * \globalvars
    *  UART_0_rxBufferHead - the start index to put data into the 
    *  software receive buffer.
    *  UART_0_rxBufferTail - the start index to get data from the 
    *  software receive buffer.
    *
    *******************************************************************************/
    uint32 UART_0_SpiUartReadRxData(void)
    {
        ...
        #if (UART_0_CHECK_RX_SW_BUFFER)
        {
            if (UART_0_rxBufferHead != UART_0_rxBufferTail)
            {
                ...
                if (UART_0_INTERNAL_RX_BUFFER_SIZE == locTail)
                {
                    locTail = 0u;
                }
                ...
                #if (UART_0_CHECK_UART_RTS_CONTROL_FLOW)
                {
                    /* Check if RX Not Empty is disabled in the interrupt */
                    if (0u == (UART_0_INTR_RX_MASK_REG & UART_0_INTR_RX_NOT_EMPTY))
                    {
                        /* Enable RX Not Empty interrupt source to continue
                        * receiving data into software buffer.
                        */
                        UART_0_INTR_RX_MASK_REG |= UART_0_INTR_RX_NOT_EMPTY;
                    }
                }
                #endif
    ...
    *******************************************************************************/
    uint32 UART_0_SpiUartGetRxBufferSize(void)
    {
        ...
        #if (UART_0_CHECK_RX_SW_BUFFER)
        {
            ...
            {
                size = (locHead + (UART_0_INTERNAL_RX_BUFFER_SIZE - UART_0_rxBufferTail));
            }
        }
        #else
        {
    ...
    *
    * \globalvars
    *  UART_0_txBufferHead - the start index to put data into the 
    *  software transmit buffer.
    *  UART_0_txBufferTail - start index to get data from the software
    *  transmit buffer.
    *
    *******************************************************************************/
    void UART_0_SpiUartWriteTxData(uint32 txData)
    {
        ...
        #if (UART_0_CHECK_TX_SW_BUFFER)
        {
            ...
            else
            {
                ...
                if (0u == (UART_0_INTR_TX_MASK_REG & UART_0_INTR_TX_NOT_FULL))
                {
                    /* Enable TX Not Full interrupt source to transmit from software buffer */
                    UART_0_INTR_TX_MASK_REG |= (uint32) UART_0_INTR_TX_NOT_FULL;
                }
            }
        }
        #else
