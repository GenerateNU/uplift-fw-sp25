...
#define PWM_MAX_DUTY 100  // Max PWM duty cycle (100%)
#define PWM_MIN_DUTY 0    // Min PWM duty cycle (0%)
...
void SetMotorSpeed(uint8_t dutyCycle) {
    ...
    PWM_BACK_WriteCompare((dutyCycle * PWM_BACK_ReadPeriod()) / 100);
}

void handle_idle_state() {
    SetMotorSpeed(0); // Stop motor in IDLE state
}

void handle_raise_legs_state() {
    SetMotorSpeed(75); // Move motor at 75% speed    
}

void handle_lower_legs_state() {
    SetMotorSpeed(50); // Move motor at 50% speed
}

void handle_raise_seat_state() {
    SetMotorSpeed(60); // Move motor at 60% speed
}

void handle_lower_seat_state() {
    SetMotorSpeed(40); // Move motor at 40% speed
}

void handle_panic_state() {
    SetMotorSpeed(0); // Stop motor in PANIC state
}
...
int main(void)
{
    ...
    PWM_BACK_Start(); // Initialize PWM module
